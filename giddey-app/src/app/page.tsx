		...
		<div className='min-h-screen flex flex-col' style={{ background: 'linear-gradient(180deg, #0a0e17 0%, #111827 100%)' }}>
			...
			<main className='flex-1 flex flex-col items-center justify-center px-4 py-8'>
				...
				<div className='text-center mb-8 animate-fade-in'>
					...
					<p className='text-sm text-white/40 mt-2 max-w-xs mx-auto'>Draft 9 players. Build chemistry. Score big.</p>
				</div>

				{/* CTA */}
				<Link href='/draft' className='btn-primary text-white text-lg font-bold px-10 py-4 rounded-xl no-underline mb-8 uppercase tracking-wider'>
					Start Draft
				</Link>
						...
						{tiers.map(({ tier, label, ovr }) => {
							...
							return (
								<div key={tier} className='flex flex-col items-center gap-1'>
									<div className={`w-8 h-8 rounded-md tier-${tier} flex items-center justify-center`} />
									...
									<span className='text-[7px] text-white/50 font-semibold text-center leading-tight'>{ovr}</span>
								</div>
							);
						})}
					</div>
				...
				<div className='mt-8 flex gap-4'>
					<Link href='/how-to-play' className='text-sm text-white/40 hover:text-white/70 transition-colors no-underline'>
						How to Play →
					</Link>
				</div>
			</main>
